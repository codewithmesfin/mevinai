...
export default function SSLInfo({ site, user }: PROPS) {
    ...
    const installSSLL = () => {
        ...
        api.create(payload, "/setup-ssl")
            .then(() => {
                setInstalling(false)
                show.success(`Dear ${user?.firstName}, SSL Certificate has been installed to your site ${user?.siteName} successfully.`)
                router.push("/configuration")
            })
            .catch(() => {
                show.error(`Unable to install ssl certificate to domain name ${user?.siteName} right now. Try again.`)
                setInstalling(false)
            })
    ...
    return <div className="border border-gray-200 rounded-xl p-5 md:p-6">
        {configuration.ssl ? <div className="flex  space-x-3">
            ...
            : <div className="flex  space-x-3">
                ...
                <div>
                    ...
                    <div className="mt-2">
                        <p className="text-sm text-gray-500">
                            Dear {user?.firstName}, we are unable to install SSL certificate to your Software as a Service, {user?.siteName} this time. Please try again.
                        </p>
                        <div className="py-3 max-w-[150px]">
                            <Button title={"Try again"} py="py-1.5"
                                isLoading={installing} onclick={installSSLL}
                                showNextIcon
                                disabled={installing}
